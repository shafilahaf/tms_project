...
class ShProductBarcodeMobileWizard(models.TransientModel):
    ...
    post_msg = fields.Html('Message',
                           translate=True,
                           help='Message displayed after having scan product')

    def default_sh_product_bm_is_cont_scan(self):
        if self.env.user and self.env.company:
            return self.env.company.sh_product_bm_is_cont_scan
    ...
    sh_product_bm_is_cont_scan = fields.Char(
        ...
        default=default_sh_product_bm_is_cont_scan,
        readonly=True)
    ...
    def _onchange_sh_product_barcode_mobile(self):
        ...
        if self and self.sh_product_barcode_mobile:
            ...
            if search_product:
                ...
                if self.env.company.sudo(
                ).sh_product_bm_is_qty_available and search_product.type == 'product':
                    msg += '''
                    <br/><br/>
                ...
                if self.env.company.sudo(
                ).sh_product_bm_is_virtual_available and search_product.type == 'product':
                    msg += '''
                    <br/><br/>
